package com.datting_package.Flirty_Datting_App.Chat.Videos;
...
/**
 * Created by datting_package on 6/7/2018.
 */
...
public class Chat_Send_file_Service extends Service {
    ...
    public void savePdfInFirebase() {
        ...
        StorageReference ref = reference.child("Document").child(key + ".pdf");
        ref.putFile(uri).addOnSuccessListener(taskSnapshot -> ref.getDownloadUrl().addOnSuccessListener(uri -> {
            stopSelf();
            uploadPdf(uri.toString(), key);
        }));
    }
    ...
    public void uploadPdf(String url, String key) {
        ...
        rootref.updateChildren(user_map, (databaseError, databaseReference) -> {
            ...
            rootref.updateChildren(both_user_map).addOnCompleteListener((OnCompleteListener<Void>) task -> sendPushNotification("Send a Pdf file"));
        });
    }
